...
| [`memory/`](memory/README.md) | 项目级持久问题台账：Service Definition + 模型侧记录/解决工具 | 产品：稳定 API |
...
| [`guard/`](guard/README.md) | 循环卫生守卫：操作契约提示词、建议性停滞／重复提醒，以及 `tools/execute` 截止时间强制执行器 | 产品：稳定 API |
