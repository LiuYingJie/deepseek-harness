...
行为 guard 插件监视 agent loop（智能体循环）中的无效模式，贡献 coding agent 操作契约提示词，并强制执行单次调用预算。guard 是核心服务和扩展点的自包含消费方，而非可替换能力。

| 包 | 职责 | ctx key |
|---|---|---|
| [`repeat-tool-reminder/`](repeat-tool-reminder/README.md) | 针对重复工具调用的建议性提醒 | 监听工具和 agent 事件 |
| [`exploration-hygiene/`](exploration-hygiene/README.md) | coding agent 操作契约加上探查停滞提醒 | 提示词段落，并监听工具和 agent 事件 |
