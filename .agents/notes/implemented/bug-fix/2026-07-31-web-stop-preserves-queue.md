...
`session.cancel` without `scope`, or with `scope: 'turn'`, is the Web Host API's active-turn stop for ordinary sessions. It rejects session-backed subagents with `agent-busy`; otherwise it calls `agent.cancel({ kind: 'user' }, { keepInbox: true })`, preserving pending inbox work while cooperatively aborting the current turn. The underlying option preserves queued and steering entries; the Web Queue projection continues to expose only queued entries. `scope: 'all'` is the separate Halt all action ([Web Halt all](../feature/2026-08-14-web-halt-all.md)).
...
**Add a wire option for broad versus preserving cancellation.** Rejected until the Web product has a separate “stop and clear Queue” interaction. The existing stop button has one policy: omitted `scope` and `scope: 'turn'` preserve the inbox. The Halt all composer action is the separate bulk-clear interaction and uses `scope: 'all'`; see [Web Halt all](../feature/2026-08-14-web-halt-all.md).
