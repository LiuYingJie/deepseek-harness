...
省略 `scope` 或 `scope: 'turn'` 的 `session.cancel` 是 Web Host API 面向普通会话的活动轮次停止操作。它会以 `agent-busy` 拒绝由会话支撑的 subagent；否则会调用 `agent.cancel({ kind: 'user' }, { keepInbox: true })`，在协作式中止当前轮次的同时保留待处理 inbox 工作。底层选项会保留 queued 和 steering 入队项；Web Queue 投影继续只暴露 queued 入队项。`scope: 'all'` 是独立的全部停止操作（[Web 全部停止](../feature/2026-08-14-web-halt-all.md)）。
...
**为广义取消与保留式取消添加协议选项。** 之所以否决：在 Web 产品提供独立的「停止并清空 Queue」交互之前，不需要此选项。现有停止按钮只有一项策略：省略 `scope` 与 `scope: 'turn'` 会保留 inbox。全部停止是独立的批量清空交互，使用 `scope: 'all'`；见 [Web 全部停止](../feature/2026-08-14-web-halt-all.md)。
...
Web 停止会保留已接纳的排队意图，并在取消如实结算后自动推进。不配合取消的活动工作收尾时，Queue 行可能仍保持可见；由同一 inbox 选项保留的外部 steering 可以进入下一个已接纳轮次，尽管 Web 不会在 QueueDock 中渲染 steering。全部停止是独立的批量清空与后代排空操作（[Web 全部停止](../feature/2026-08-14-web-halt-all.md)）。
